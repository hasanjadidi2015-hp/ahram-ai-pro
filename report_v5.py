...
DBS = [("اهرم", "ahram_v2_v5.db"), ("وبملت", "webmellt_v5.db"), ("شستا", "shasta_v5.db")]
today = datetime.now().strftime("%Y-%m-%d")

print("=" * 60)
print(f"📊 گزارش AHRAM AI V5 (آزمایشی) - {today}")
print("=" * 60)
...
try:
    try:
        import dashboard_v5 as dashboard
    except ImportError:
        import dashboard
    dashboard.SYMBOL_DBS = DBS
    dashboard.generate()
...
try:
    import strategy_bridge_v5 as strategy_bridge
...
try:
    ...
    # shadow_strategy.py مخصوص v5 نداره -- به‌جای مقادیر پیش‌فرض v1
    # (که فایل‌های v1 رو می‌خونه/می‌نویسه)، ورودی/خروجی/تاریخچه رو به فایل‌های
    # مخصوص v5 پوینت می‌کنیم تا با گزارش shadow نسخه‌ی اصلی قاطی نشه.
    shadow_strategy.INPUT_FILE = "ahram_strategy_data_v5.json"
    shadow_strategy.OUTPUT_FILE = "ahram_shadow_report_v5.json"
    shadow_strategy.HISTORY_FILE = "ahram_shadow_history_v5.json"
    import sys
    old_argv = sys.argv
    sys.argv = [old_argv[0]]
    try:
        shadow_strategy.main()
        print("  ✅ گزارش Shadow V5 (ahram_shadow_report_v5.json) بروز شد")
    finally:
        sys.argv = old_argv
except Exception as e:
    print(f"  ⚠️ خطا Shadow V5: {e}")

try:
    import connect_strategy_dashboard_v5 as connect_strategy_dashboard
    import sys
    old_argv = sys.argv
    sys.argv = [old_argv[0]]
    try:
        connect_strategy_dashboard.main()
        print("  ✅ داشبورد V5 (LIVE4_V5 / LIVE5) بروز شد")
    finally:
        sys.argv = old_argv
except Exception as e:
