"""
AHRAM AI PRO - پل داده برای داشبورد استراتژی + V2 + Sentiment (نسخه یکپارچه)
...
DATABASES_FALLBACK = DATABASES  # دیگه نیازی به fallback جدا نیست، همه یکی شدن
...
def build_symbol_data(name, db_path):
    # fallback احتیاطی (دیگه لازم نیست چون یکی شدن)
...
def build_payload():
    return {
        ...
        "source": "AHRAM AI PRO + V2 + Sentiment",
        "read_only": True,
