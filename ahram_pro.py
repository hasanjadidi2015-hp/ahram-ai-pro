...
def analyze_options(symbol_config, stock_action, stock_confidence, stock_price):
    ...
    try:
        ...
        if get_best_option:
            if stock_action in ("BUY", "STRONG BUY"):
                opt_signal_type = "BUY_CALL"
            elif stock_action in ("SELL", "STRONG SELL"):
                opt_signal_type = "BUY_PUT"
            else:
            ...
            if option:
                ...
                state.log(f"  ✅ آپشن انتخاب شد: {option.get('symbol')} (اسنپ‌شات: {option.get('snapshot_time')})")
